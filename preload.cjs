...
contextBridge.exposeInMainWorld('fw', {
  load: () => call('mov:load'),
  add: (move) => call('mov:add', move),
  update: (move) => call('mov:update', move),
  remove: (id) => call('mov:remove', id),
  exportCsv: (moves, suggestedName) => call('mov:export-csv', moves, suggestedName),
  /** El respaldo no recibe movimientos: el main los lee del disco. */
  exportAll: () => call('mov:export-all'),
  /** El diálogo y la lectura pasan en el main; acá no hay fs. */
  importBackup: () => call('mov:import'),
});
